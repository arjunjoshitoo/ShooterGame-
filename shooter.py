...
pygame.display.set_caption("Arjun's Game")
...
class Soldier(pygame.sprite.Sprite):
    ...
    def update_animation(self):
        ...
        self.image = self.animation_list[self.action][self.frame_index]
        #check if enough time as passed since the last update 
        if pygame.time.get_ticks() - self.update_time > ANIMATION_COOLDOWN:
            self.update_time = pygame.time.get_ticks()
            self.frame_index += 1 
        #if the animation has run out then reseet back to the start 
...
class Grenade(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        #check colllision with the screen
...
while run:
    ...
    for x in range(player.ammo):
        screen.blit(bullet_img, (70 + (x * 10), 40))
    #show grenades 
    draw_text("GRENAGES: ", font, WHITE, 10, 60)
    for x in range(player.grenades):
        screen.blit(grenade_img, (100 + (x * 14), 62))
